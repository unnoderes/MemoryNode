...
批准后的记忆可以按关键词搜索。英文和其他非 CJK 查询仍使用 SQLite FTS5 关键词匹配；中文短语支持本地内容子串回退，例如内容含有 `审核者批准后才能成为` 时，搜索 `审核者批准` 即可找到。已撤销、已过期或被替换的内容默认不会混入结果。
...
确认 `uv`/`uvx` 已加入 `PATH` 后，将下面配置粘贴到兼容的 MCP 客户端。首次使用时，`uvx` 会下载并缓存包；随后它会启动 stdio MCP，并在已配置的端口与进程记录安全时复用已验证的本地 MemoryNode API，或启动受管理的 API 和治理控制台。标准输出保留给 MCP 协议，请不要把调试日志写到标准输出。
...
该命令是显式的仅 stdio 模式：不会启动 HTTP MCP、生成 HTTP 令牌、修改端口、终止未知进程或创建竞争运行时。它需要包含此功能的 PyPI 正式发布版本；在该版本发布前，请使用从源码安装的包。

如果希望仅在本次引导启动受管理的 API 和控制台时自动打开默认浏览器，请在 `--ensure-api` 后加上 `--open-console`。复用现有 API 或显式 `MEMORYNODE_API_URL` override 时不会打开浏览器。

如果 MCP 客户端显式设置 `MEMORYNODE_API_URL`，引导只接受精确的 `http://127.0.0.1:<port>` 源。只有当其 `/health` 响应确认是 MemoryNode 时才会复用该精确地址；否则会安全拒绝，不会启动或重定向到其他实例。

已安装的包可使用同样安全的引导方式，不必经过 `uvx`：
...
请在本地治理控制台（或文档说明的本地环境变量）中配置模型服务和密钥，绝不要把它们写入 MCP 配置或命令参数。`memory_propose` 只会创建待审核提案；控制台仍是人工批准和拒绝的界面。默认可用工具包括：提交记忆提案、搜索、读取、解释、列出记忆和提交反馈。批准、拒绝、撤销、替换、设置到期等会改变治理状态的工具，默认隐藏，只有本地管理员在配置中显式开启后才会暴露。
